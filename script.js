const API_BASE = "https://greenishmart-backend.vercel.app/api";
  ...
  try {
    ...
    if (data.ok) {
      ...
      if (productCard) {
        ...
        setTimeout(() => {
          ratingContainer.style.transform = "scale(1)";
        }, 200);
      }
    } else {
      ...
      grid.innerHTML = products
        .map((product) => {
          ...
          return `
            ...
            product.name
          }" onerror="this.src='https://placehold.co/400?text=Image+Not+Found'">
                        <div class="product-actions">
                            <button class="action-btn like-btn" data-id="${
                              product.id
                            }" title="Like">
                                <i class="far fa-heart"></i> <span class="like-count">${
                                  product.likes || 0
                                }</span>
                            </button>
                    ...
                    <div class="product-info">
                        ...
                        <div class="product-price" style="display: flex; flex-direction: column; gap: 4px;">
                            <span class="current-price" style="font-size: 1.1rem; color: var(--primary-dark); font-weight: 700;">$${parseFloat(
                              product.price_usd
                            ).toLocaleString()}</span>
                            <span class="naira-price" style="font-size: 0.9rem; color: var(--gray); font-weight: 500;">₦${parseFloat(
                              product.price_ngn
                            ).toLocaleString()}</span>
                        </div>
...
function initSearch() {
  ...
  document
    .querySelectorAll(".sidebar-list a, .categories-dropdown a, .category-card")
    .forEach((link) => {
...
function updateAuthUI() {
  ...
  if (isLoggedIn && userStr) {
    try {
      ...
      const authText = document.querySelector(
        ".user-actions .action-item .action-text"
      );
      if (authText) {
        authText.querySelector("span").textContent = `Hello, ${
          user.firstName || "User"
        }`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.body.addEventListener("click", async function (e) {
    ...
    if (e.target && e.target.classList.contains("add-to-cart")) {
      ...
      const titleElement =
        productCard.querySelector(".product-title a") ||
      ...
      const cartTotalElement = document.querySelector(
        ".cart .action-text strong"
      );
      if (cartTotalElement) {
        // simple parsing, removing $ and commas
        const currentTotal =
          parseFloat(cartTotalElement.textContent.replace(/[$,]/g, "")) || 0;
  ...
  if (window.innerWidth <= 768 && categoriesBtn && categoriesDropdown) {
    categoriesBtn.addEventListener("click", function (e) {
      ...
      categoriesDropdown.style.display =
        categoriesDropdown.style.display === "flex" ? "none" : "flex";
    });

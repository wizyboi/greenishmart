...
/**
 * @note In theory, writeln and all of its callers should be async to allow for sending out socket messages before the entire test execution
 * has ended. However, Mocha by design does not allow asynchronous runner methods (https://github.com/cypress-io/cypress/issues/7139). For
 * an async implementation, see plugins/JavaScriptLanguage/resources/helpers/base-test-reporter/intellij-tree.js.
 */
function Tree(write) {
  /**
   * @type {Function}
...
Node.prototype.getInitMessage = function (running) {
  ...
  text += ' nodeId=\'' + util.escapeAttributeValue(this.id);
  var parentId = this.parent ? this.parent.id : 0;
  text += '\' parentNodeId=\'' + util.escapeAttributeValue(parentId);
...
TestNode.prototype.addStdErr = function (err) {
  if (util.isString(err)) {
    var text = '##teamcity[testStdErr nodeId=\'' + util.escapeAttributeValue(this.id)

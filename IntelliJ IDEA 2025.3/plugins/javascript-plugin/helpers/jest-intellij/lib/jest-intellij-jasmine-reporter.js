...
(function () {
  ...
  if (setupTestFrameworkScriptFile) {
    try {
      ...
      util.warn("Failure from " + setupTestFrameworkScriptFile + ': ' + e.message + '\n' + e.stack);
    }
  }
...
JasmineReporter.prototype.jasmineStarted = util.safeAsyncFn(async function (options) {
  this.tree.addTotalTestCount(this.getTotalNotDisabledSpecCount());
  await writer.flush();
});

JasmineReporter.prototype.suiteStarted = util.safeAsyncFn(async function (result) {
  const locationPath = util.getTestLocationPath(this.currentSuiteNode, result.description,
...
JasmineReporter.prototype.suiteDone = util.safeAsyncFn(async function (result) {
  const suiteNode = this.currentSuiteNode;
  if (suiteNode == null) {
    return util.warn('No current suite to finish');
  }
  if (suiteNode.name !== result.description) {
    return util.warn('Suite name mismatch, actual: ' + suiteNode.name + ', expected: ' + result.description);
  }
  if (!jestIntellijUtil.isCreatedState(suiteNode)) {
...
function startAncestorSuites(suite, root) {
  if (jestIntellijUtil.isCreatedState(suite) && suite !== root) {
 ...
 */
JasmineReporter.prototype.specStarted = util.safeAsyncFn(async function (result) {
  ...
  if (this.nodeById[result.id] != null) {
    util.warn('jasmine error, specStarted with not unique result.id: ' + result.id)
  }
...
function passedTime(startTime) {
  if (typeof startTime.millis === 'number') {
    try {
      ...
      util.warn('Failed to call Date.now() on specDone: ' + e.message);
    }
  }
  else {
    util.warn('Failed to call Date.now() on specStarted: ' + startTime.error);
  }
}
 ...
 * @param {jasmine.Result} result
 */
JasmineReporter.prototype.specDone = util.safeAsyncFn(async function (result) {
  const specNode = this.nodeById[result.id];
  if (specNode == null) {
    if (this.isSpecDisabled(result)) {
      return;
    }
    return util.warn('Cannot find specNode by id ' + result.id);
  }
...
JasmineReporter.prototype.jasmineDone = util.safeAsyncFn(async function () {
  jestIntellijUtil.addEmptyTestResultIfNeeded(this.tree.root);

...
function Tree(idPrefix, write, rootId) {
...
Tree.prototype.createRootNode = function () {
  return new TestSuiteNode(this, this.rootId || 0, null, 'hidden root', null, null);
};
...
Tree.prototype.sendMessage = function (commandName, parameters) {
  let messageStr = '##teamcity[' + commandName;
  for (const [key, value] of Object.entries(parameters)) {
    messageStr = messageStr + ' ' + escapeAttributeValue(key) + "=\'" + escapeAttributeValue(value) + "\'"
  }
  messageStr += ']';
  this.writeln(messageStr);
};
...
Node.prototype.getInitMessage = function (running) {
  ...
  text += ' nodeId=\'' + escapeAttributeValue(this.id);
  var parentId = this.parent ? this.parent.id : 0;
  text += '\' parentNodeId=\'' + escapeAttributeValue(parentId);
...
Node.prototype.getStartMessage = function () {
  ...
  if (this.state === NodeState.REGISTERED) {
    ...
    text += commandName;
    text += ' nodeId=\'' + escapeAttributeValue(this.id);
    // required for jetbrains.buildServer.messages.serviceMessages.BaseTestMessage.getTestName.
    text += '\' name=\'' + escapeAttributeValue(this.name);
    text += '\' running=\'true\']';
    return text;
  }
...
/**
 * Returns child nodes by its name.
 * @param {string} childName
 * @return {Node[]}
 */
TestSuiteNode.prototype.findChildNodesByName = function(childName) {
  const result = getChildrenByName(this, childName);
  if (Array.isArray(result)) {
    return result;
  }
...
function TestNode(tree, id, parent, name, type, locationPath) {
   ...
   * @private
   */
  this.printExpectedAndActualValues = true;
}
...
TestNode.prototype.setOutcome = function (outcome, durationMillis, failureMsg, failureDetails,
  ...
  if (!failureMsg) {
    if (outcome === TestOutcome.SKIPPED) {
      this.failureMsg = 'Skipped test \'' + this.name + '\'';
    }
    else if (outcome === TestOutcome.FAILED || outcome === TestOutcome.ERROR) {
      this.failureMsg = 'Failure cause not provided for \'' + this.name + '\'';
    }
  }
};

TestNode.prototype.setPrintExpectedAndActualValues = function (printExpectedAndActualValues) {
  this.printExpectedAndActualValues = printExpectedAndActualValues;
};
...
TestNode.prototype.getExtraFinishMessageParameters = function () {
  ...
  if (!this.printExpectedAndActualValues) {
    params += ' printExpectedAndActual=\'false\'';
  }
 ...
 * @param {string} out
 */
TestNode.prototype.addStdOut = function (out) {
  if (isString(out)) {
    this.tree.sendMessage('testStdOut', {nodeId: this.id, name: this.name, out: out});
  }
};
...
TestNode.prototype.addStdErr = function (err) {
  if (isString(err)) {
    this.tree.sendMessage('testStdErr', {nodeId: this.id, name: this.name, out: err});
  }
};

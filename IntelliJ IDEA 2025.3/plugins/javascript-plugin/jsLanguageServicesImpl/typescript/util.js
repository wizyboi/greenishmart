...
function initCommandNames(TypeScriptCommandNames) {
    ...
    TypeScriptCommandNames.IDEOpenExternalProject = "ideOpenExternalProject";
    TypeScriptCommandNames.IDEGetMainFileErrors = "ideGetMainFileErr";
    TypeScriptCommandNames.IDEGetProjectErrors = "ideGetProjectErr";
    TypeScriptCommandNames.IDEGetProjectsInfo = "ideProjectInfo";
    (0, tsc_plugin_adapter_1.initCommandNamesForSessionProvider)(TypeScriptCommandNames);
    if (TypeScriptCommandNames.ReloadProjects == undefined) {
        TypeScriptCommandNames.ReloadProjects = "reloadProjects";
    }
    if (TypeScriptCommandNames.Status == undefined) {
        TypeScriptCommandNames.Status = "status";
    }
}
...
function statusResponse(tsImpl) {
    return {
        responseRequired: true,
        response: {
            version: tsImpl.version,
...
/** copied from tscplugin/index.ts */
...
function getDefaultSessionClass(ts_impl, host, defaultOptionsHolder) {
    var DefaultSessionExtension = /** @class */ (function (_super) {
        ...
        DefaultSessionExtension.prototype.compileFileEx = function (req) {
            ...
            try {
                var isIncludeErrors = req.richResponse;
                ...
                return { response: { generatedFiles: compileInfo.getOutFiles(),
                        diagnostics: diagnostics_1, processedFiles: processedFiles_1 }, responseRequired: true };
            }
            finally {
...
function findArgument(argumentName) {
    ...
    var index = argv.indexOf(argumentName);
    return index >= 0 && index < argv.length - 1
        ? argv[index + 1]
        : undefined;
}
exports.findArgument = findArgument;
function findArgumentStringArray(argName) {
    var arg = findArgument(argName);
    if (arg === undefined) {
        return [];
    }
